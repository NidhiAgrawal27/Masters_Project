...
def plot_edges_gaussian(x, y, cur, wt, xlabel, ylabel, fig_file_name):
    ...
    if wt != 'weighted':
        plt.plot(x_unique, x_unique*(x_unique-1)/2, color = uzh_colors['green'], linewidth = 0.85, label = 'Theoretical max num of edges')
    plt.plot(x_unique, x_unique-1, color = uzh_colors['red'], linewidth = 0.85, label = 'Theoretical min num of edges')
    plt.legend()
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.title(title)
...
def plotPowerLaw(df, cur, heuristic, fig_file_name, xmin= None, xmax = None):
    ...
    fig = fit.plot_ccdf(color= uzh_colors["red"], label= heuristic + ', '+ r'$\alpha$ = ' + alpha + ' in range c = [%.0f, %.0f]'%(xmin,xmax))
    # fit.power_law.plot_ccdf( color= uzh_colors["blue"],linestyle='--',label='fit ccdf',ax=fig)
    plt.xscale('log')
    plt.yscale('log')
    plt.minorticks_off()
    plt.ylabel('Complementary cumulative distribution function 1 - P(c)')
