...
def main():
    ...
    for tq in tqdm(range(1), desc = cur + ' ' + heuristic + ' tqdm() Progress Bar: '):
        ...
        # map vertext and edge properties and write csv files for address_id, edge and components data
        ...
        if os.path.isfile(fig_dir + 'density_plot.png') == False:
            if not isinstance(df_components, type(None)):
                if os.path.isfile(dir_generated_files + 'components.csv') == False:
                    components = compute_components.compute_components(graph_of_correspondences)
                    df_components = pd.DataFrame.from_dict(components, orient='columns')
                    df_components.to_csv(dir_generated_files + 'components.csv', index=False)
                else: df_components = pd.read_csv(dir_generated_files + 'components.csv')
        ...
        else: print(cur + ' ' + heuristic + ': density_plot.png exists')

        if os.path.isfile(fig_dir + 'powerlaw_plot.png') == False:
            if not isinstance(df_components, type(None)):
                if os.path.isfile(dir_generated_files + 'components.csv') == False:
                    components = compute_components.compute_components(graph_of_correspondences)
                    df_components = pd.DataFrame.from_dict(components, orient='columns')
            ...
            plotPowerLaw(df_components['num_of_addrs'], cur, fig_dir + 'powerlaw_plot.png')
            print('\n'+ cur + ' ' + heuristic + ': powerlaw_plot.png completed')
        else: print(cur + ' ' + heuristic + ': powerlaw_plot.png exists')


        # visualize network
        if vis == 'yes':
            if os.path.isfile(fig_dir + 'correspondence_network' + '.pdf') == False:
                plt =draw.graph_draw(graph_of_correspondences, vertex_text=graph_of_correspondences.vertex_index, 
